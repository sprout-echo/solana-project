...
function genLibraryImport(list, from) {
...
function genSideEffectsImport(from) {
  return `import '${from}';
`;
}
...
function resolveComponents(config) {
  ...
  allComponents.forEach(async (item) => {
    ...
    const filePath = from !== iconLibraryName ? `${libraryName}/es/components/${dir}/index.mjs` : from;
    addComponent({
      export: name,
      name: alias || name,
...
async function resolveInjection(config) {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      s.replace(directivesRegExp, (full, name) => {
        ...
        if (directiveConfig) {
          const [directive, styles2] = directiveConfig;
